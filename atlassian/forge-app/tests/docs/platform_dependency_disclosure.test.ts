...
describe('Platform Dependency Disclosure', () => {
  ...
  describe('Uninstall behavior disclaimer', () => {
    ...
    it('No claim "FirstTry deletes data on uninstall"', () => {
      ...
      docs.forEach(doc => {
        // Should NOT claim FirstTry DOES delete/purge on uninstall (positive claim)
        expect(doc).not.toMatch(/FirstTry.*deletes.*on.*uninstall|FirstTry.*purges.*on.*uninstall|FirstTry.*will.*delete.*uninstall/i);
      });
    });
